...
// 🌙 DARK MODE — defined later in the file (toggleDark with localStorage)
...
const deptEl = document.getElementById("bookDept");
...
async function loadBooks() {
    ...
    books.forEach(b => {
        ...
        const safeDept = b.dept.replace(/'/g, "\\'").replace(/"/g, '&quot;');
        ...
        <div class="card">
            ...
            <button onclick="editBook('${b.id}', '${safeTitle}', '${safeAuthor}', '${safeDept}', '${b.qty}')">
                ✏️ Edit
            </button>
        </div>`;
    });
...
async function loadRequests() {
    ...
    const btn = document.getElementById("requestsNavBtn");

    if (data.length === 0) {
        div.innerHTML = `<p>No pending requests 📭</p>`;
        if (btn) btn.innerHTML = "📩 Requests";
        return;
    }

    if (btn) btn.innerHTML = `📩 Requests <span class="badge">${data.length}</span>`;
    ...
    data.forEach(r => {
        ...
        toast("Failed to load requests ⚠️");
    }
}
...
async function loadIssued() {
    ...
    books.forEach(b => {
        ...
        console.error("loadIssued error:", err);
        toast("Failed to load issued books ⚠️");
    }
}
